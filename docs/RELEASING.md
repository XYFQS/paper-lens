...
| 文件                               | 用途                        |
| ---------------------------------- | --------------------------- |
| `dist/paper-lens-1.0.0.xpi`        | 用户直接安装的插件包        |
| `dist/paper-lens-1.0.0-source.zip` | 完整源码与文档              |
| `dist/SHA256SUMS.txt`              | 两个压缩包的 SHA-256 校验值 |

`dist/` 被 Git 忽略，因此 VS Code 同步源码不会上传安装包。这三个文件应作为 Release 附件上传。

## 2. 创建 Release

1. 打开仓库的 **Releases**，点击 **Draft a new release**（首次可能显示 **Create a new release**）。
2. 在 **Choose a tag** 中创建 `v1.0.0`，**Target** 选择刚推送源码所在的分支。
3. 标题填写 `文献透镜 Paper Lens v1.0.0`，填写下方发布说明。
...
```markdown
文献透镜首个正式版本，支持 Windows / Zotero 9.0.6。
...
安装：下载 paper-lens-1.0.0.xpi，在 Zotero 中选择“工具 → 插件 → 齿轮 → 从文件安装插件”。无需解压。
...
更新 `addon/manifest.json` 和 `package.json` 的版本号，完成测试、提交推送、重新打包，再创建对应标签及 Release，例如 `1.0.1` / `v1.0.1`。如果 `v1.0.0` 已正式发布，后续改动应使用新版本号。
